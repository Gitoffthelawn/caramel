...
async function handleGet(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        return apiResponse(req, res, 200, 'sources', sourcesWithMetrics)
    } catch (error) {
        console.error('Error fetching sources:', error)
        return apiResponse(req, res, 500, 'Error fetching sources.', null)
    }
}

async function handlePost(req: NextApiRequest, res: NextApiResponse) {
    try {
        const { website }: CreateSourceBody = req.body
        if (!website) {
            return apiResponse(req, res, 400, 'Missing required fields.', null)
        }
        //test is it's a valid website URL
        const isValidWebsite = true
        if (!isValidWebsite) {
            return apiResponse(req, res, 400, 'Invalid website URL.', null)
        }
        ...
        return apiResponse(req, res, 500, 'Error creating source.', null)
    }
}
