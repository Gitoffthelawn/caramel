...
export async function GET(req: NextRequest) {
  try {
    const sources = await prisma.source.findMany({ where: { status: 'ACTIVE' }, include: { coupons: true } })
    const sourcesWithMetrics = sources.map(src => {
      ...
      const successRate = totalUsed + totalFail === 0 ? 0 : (totalUsed / (totalUsed + totalFail)) * 100
      return {
        id: src.id,
        source: src.source,
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!website) return nextApiResponse(req, 400, 'Missing required fields.', null)
    await prisma.source.create({ data: { source: website, status: 'REQUESTED' } })
    return nextApiResponse(req, 200, 'Source submission requested successfully!')
  } catch (error) {
