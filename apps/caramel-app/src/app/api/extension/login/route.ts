...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!process.env.JWT_SECRET) return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
    const token = jwt.sign({ sub: user.id, email: user.email }, process.env.JWT_SECRET, { expiresIn: '1d' })
    return NextResponse.json({ token, username: user.username, image: user.image || null })
  } catch {
