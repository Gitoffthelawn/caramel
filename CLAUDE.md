...
- pnpm@9 monorepo, two packages. `apps/caramel-app` = Next.js 16 App Router: Prisma → auth/user Postgres; the coupon catalog lives in a SECOND, externally-owned Postgres (`caramel_coupons`, written only by an out-of-repo Python service) read via porsager `postgres` in `src/lib/couponsDb.ts` — every read zod-parsed by `parseCouponRows` (schema drift throws loudly).
