...
function waitForElement(sel, timeout = 4000) {
    return new Promise((res, rej) => {
        ...
        const mo = new MutationObserver(() => {
            if (document.querySelector(sel)) { mo.disconnect(); res('appeared'); }
        });
        mo.observe(document.documentElement, { childList:true, subtree:true });
        setTimeout(() => { mo.disconnect(); rej(`waitForElement timeout (${sel})`); }, timeout);
    });
}
function waitForTextChange(el, timeout = 3000) {
    return new Promise((res, rej) => {
        ...
        const mo = new MutationObserver(() => {
            if (el.textContent !== start) { mo.disconnect(); res('text-changed'); }
        });
        mo.observe(el, { characterData:true, childList:true, subtree:true });
        setTimeout(() => { mo.disconnect(); rej('waitForTextChange timeout'); }, timeout);
    });
}
    ...
    return new Promise(resolve => {
        ...
        window.fetch = (...args) => {
            ...
            if (url.includes('/apply-discount')) {
                p.finally(() => { window.fetch = orig; resolve('network-reply'); });
            }
            return p;
        };
    });
...
function getPrice(selector, { returnLargest } = {}) {
    ...
    if (!el) { log('getPrice: element NOT found', selector); return NaN; }

    const regex = /(?:[A-Z]{1,3}\s?)?[$£€]\s?\d{1,3}(?:,\d{3})*(?:\.\d+)?/g;
    const prices = (el.innerText.match(regex) || []).map(t =>
        parseFloat(t.replace(/[^0-9.]/g, '')),
    );
    if (!prices.length) { log('getPrice: no price found'); return NaN; }
...
async function isCheckout() {
    ...
    if (!rec) return false;
    if (document.querySelector(rec.couponInput) || document.querySelector(rec.showInput)) return true;
    try { await waitForElement(`${rec.couponInput},${rec.showInput}`, 3000); }
    catch (e) { log(e); }
...
async function applyCoupon(code, rec) {
    ...
    try {
        ...
        if (rec.dismissButton) {
            ...
            if (btn) { btn.click(); await sleep(180); log('Popup dismissed'); }
        }
        ...
        if (!input && rec.showInput) {
            ...
            if (showBtn) {
                showBtn.click();
                try { await waitForElement(rec.couponInput, 3000); }
                catch (e) { log(e); }
                input = document.querySelector(rec.couponInput);
            }
        }
        const applyBtn = document.querySelector(rec.couponSubmit);
        if (!input || !applyBtn) { log('Input / apply button missing'); return { success:false }; }
        ...
        const priceEl = document.querySelector(rec.priceContainer) ||
            document.getElementById(rec.priceContainer.match(/\[id=['"]([^'"]+)['"]\]/)?.[1] || '');
        if (priceEl && rec.domain !== 'amazon.com') waiters.push(waitForTextChange(priceEl, 3000));
...
async function fetchCoupons(site, kw) {
    ...
    try {
        ...
        return d;
    } catch (e) { log('fetchCoupons error', e); return []; }
}
...
async function startApplyingCoupons(rec) {
    ...
    if (!coupons.length) { showFinalModal(0, null, 'No coupons found.'); return; }

    const original = getPrice(rec.priceContainer, { returnLargest:true });
    let bestSave = 0, bestCode = null;
    ...
    for (let i = 0; i < coupons.length; i++) {
        ...
        if (res.success) {
            ...
            log(`✓ ${code} saved ${diff}`);
            if (diff > bestSave) { bestSave = diff; bestCode = code; }
        } else { log(`✗ ${code} no savings`); }
    }
...
window.addEventListener('message', ev => {
    ...
    if (ev.data?.token) {
        currentBrowser.storage.sync.set(
            { token: ev.data.token, user: { username: ev.data.username || 'CaramelUser', image: ev.data.image } },
            tryInitialize,
        );
    }
});
